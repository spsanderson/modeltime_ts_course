...
```{r}
transactions_product_page_sessions_weekly_tbl <- left_join(
...
```{r}
transactions_product_page_sessions_weekly_tbl %>%
    pivot_longer(cols = -purchased_at) %>%
    group_by(name) %>%
...
log_standardized_transactions_product_page_tbl %>%
    plot_acf_diagnostics(.date_var = purchased_at
                         , .value = revenue
                         , .ccf_vars = contains("session")
                         , .show_ccf_vars_only = TRUE
                         , .facet_ncol = 2)
