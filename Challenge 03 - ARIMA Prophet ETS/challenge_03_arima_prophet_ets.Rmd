...
- What are the ARIMA orders? 0,1,1
    - How many lags were used? 0
    - Was differencing performed? 1
    - Were any lagged error features used? 1
- Did modeltime auto-generate a frequency for ARIMA? Yes
- Is this a seasonal model? No
...
- Does adding the seasonality seem to improve the ARIMA model? No
...
Questions:
- Does the added fourier term seem to improve the AIC of the model? Yes
...
```{r}
# CODE HERE
model_fit_7_prophet_yearly <- prophet_reg(seasonality_yearly = TRUE) %>%
    set_engine("prophet") %>%
    fit(revenue ~ purchased_at, data = training(splits))
...
```{r}
# CODE HERE
model_fit_8_events <- prophet_reg() %>%
    set_engine("prophet") %>%
